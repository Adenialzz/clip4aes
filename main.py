...
from models import CLIP4AesFormer, BLIP4AesFormer
...
def get_args():
    ...
    parser.add_argument("--data-root", type=str, default="/home/song/AVA/")
    ...
    parser.add_argument("--arch", type=str, default='resnet50', choices=timm.list_models(pretrained=True)+clip.available_models()+['blip'])
    cfg = parser.parse_args()
    return cfg

def main(cfg):
    if cfg.arch in clip.available_models():
        model = CLIP4AesFormer(cfg.arch, cfg.device, out_len=10, fc_bias=cfg.fc_bias)
        ...
        model = BLIP4AesFormer(cfg.device, out_len=10, fc_bias=cfg.fc_bias)
        if not cfg.fine_tune:
            freeze_weights(model, ['aes_fc'], None)
        pipeline = transforms.Compose([transforms.RandomHorizontalFlip(), model.preprocess])
    else:
    ...
    train_set = AVADataset(csv_file=osp.join(cfg.data_root, 'csvFiles/trainval_mlsp.csv'), root_dir=osp.join(cfg.data_root, 'shortEdge256/all_images/'), transform=pipeline)
    val_set = AVADataset(csv_file=osp.join(cfg.data_root, 'csvFiles/test_mlsp.csv'), root_dir=osp.join(cfg.data_root, 'shortEdge256/all_images/'), transform=pipeline)
